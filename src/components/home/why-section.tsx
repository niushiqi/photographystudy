      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid gap-6 md:grid-cols-2 mb-16">
          {advantages.map((advantage, index) => (
            <motion.div
              key={advantage.title}
              className="bg-card/80 border rounded-xl overflow-hidden shadow-sm"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              ...
              whileHover={{ y: -5 }}
            >
              <div className={`p-6 relative overflow-hidden ${index % 2 === 0 ? "bg-purple-900" : "bg-purple-800"}`}>
        ...
        >
          <div className="bg-card/80 border rounded-xl p-8 relative overflow-hidden shadow-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {benefits.map((benefit, index) => (
                <div 
                  key={index} 
                  className="relative bg-background/80 rounded-lg border p-4 text-center hover:border-purple-800 transition-colors overflow-hidden group"
                >
                  <div className="relative z-10">
                    <div className="mb-3 flex justify-center">
                      <div className="w-8 h-8 rounded-full bg-purple-900/20 flex items-center justify-center text-purple-400">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
                  ...
                  <div className="absolute bottom-0 left-0 w-full h-0 bg-purple-800 opacity-10 transition-all duration-300 group-hover:h-full" />
                </div>
              ))}
            </div>
          </div>
